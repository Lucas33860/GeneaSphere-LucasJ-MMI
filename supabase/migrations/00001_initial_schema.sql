...
-- parentage & spouses
create policy "parentage_select" on public.parentage for select using (auth.role() = 'authenticated');
create policy "parentage_insert" on public.parentage for insert with check (
  exists (select 1 from public.profiles where id = auth.uid() and is_approved)
);
create policy "parentage_delete" on public.parentage for delete using (
  exists (select 1 from public.profiles where id = auth.uid() and is_admin)
);

create policy "spouses_select" on public.spouses for select using (auth.role() = 'authenticated');
create policy "spouses_insert" on public.spouses for insert with check (
  exists (select 1 from public.profiles where id = auth.uid() and is_approved)
);
create policy "spouses_delete" on public.spouses for delete using (
  exists (select 1 from public.profiles where id = auth.uid() and is_admin)
);
...
begin
  ...
  values (
    ...
    is_first    -- premier inscrit = approuvé automatiquement
  );
  return new;
end;
$$;
